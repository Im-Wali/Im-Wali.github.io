...
## 서론
  * **"관계형 데이터 모델링 노트"** 책을 통해 진행한 이론 스터디 일곱번째 시간이다.
  <br/>
    비정규화에 대한 이야기들은 많다. 해당 책에서는 비정규화는 정규화를 하지 않는 것이 아니라 정규화를 진행한 후 정규화를 전략적인 판단에 의해 풀어내는 것을 비정규화라고 이야기한다.
  <br/>
...
## 비정규형의 단점
  ...
  **2) 쓰기 성능 저하**
  * 비정규화를 사용하는 이유는 읽기 성능을 향상시키기 위해서이다. 이와 다르게 쓰기 성능은 떨어지기 때문에, 이와 같은 쓰기 성능이 문제가 될 수 있다.
  <br/>
...
## 비정규화 방법 - 추출 속성
  ...
  * 추출 속성은 주로 하위 엔티티에서 많은 데이터(인스턴스)를 읽어서 연산한 후의 값을 상위 엔티티의 속서으로 가져다 놓는다. 따라서 주로 상위 엔티티의 속성 값을 하위 엔티티에 가져다 놓는 중복 속성과 달리 추출 속성은 성능에 많은 영향을 미친다.
  <br/>
